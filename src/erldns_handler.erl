...
init([]) ->
  %logger:info("Initialized the handler_registry"),
  {ok, #state{handlers=[]}}.

handle_call({register_handler, RecordTypes, Module}, _, State) ->
  %logger:info("Registered handler ~p for types ~p", [Module, RecordTypes]),
...
handle(BadMessage, {_, Host}) ->
  logger:error("Received a bad message: ~p from ~p", [BadMessage, Host]),
  BadMessage.
...
handle(Message, Host, _) ->
  %logger:debug("Questions: ~p", [Message#dns_message.questions]),
  ...
  case application:get_env(erldns, catch_exceptions) of
    ...
    _ ->
      try erldns_resolver:resolve(Message, AuthorityRecords, Host) of
        ...
        Exception:Reason ->
          logger:error("Error answering request: ~p (~p)", [Exception, Reason]),
