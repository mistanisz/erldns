      ...
      lists:foreach(
        fun(JsonZone) ->
            ...
            case erldns_zone_cache:put_zone(Zone) of
              {error, Reason} -> logger:error("Failed to load zone ~p: ~p", [JsonZone, Reason]);
              _ -> ok
            end
        end, JsonZones),
      logger:info("Loaded ~p zones", [length(JsonZones)]),
      {ok, length(JsonZones)};
    {error, Reason} ->
      logger:error("Failed to load zones: ~p", [Reason]),
      {err, Reason}
  end.
